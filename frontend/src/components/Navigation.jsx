    ...
    <nav className="sticky top-0 z-50 border-b border-white/10 bg-slate-950/70 backdrop-blur-xl">
      <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-1 sm:gap-2 lg:justify-center">
            {NAV_LINKS.map((link) => (
              <NavLink
                key={link.to}
                to={link.to}
                className={({ isActive }) =>
                  `rounded-full px-2 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm transition ${
                    isActive
          ...
          <Link
            to="/map"
            className="inline-flex items-center justify-center rounded-full bg-cyan-400 px-3 sm:px-5 py-1.5 sm:py-2.5 text-xs sm:text-sm font-semibold text-slate-950 transition hover:bg-cyan-300"
          >
            Open map
          </Link>
        </div>
