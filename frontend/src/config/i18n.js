/**
 * AdastraSky Frontend - Configuración i18next
 * Soporte multiidioma: ES, EN, DE
 */

import i18n from 'i18next';
import { initReactI18next } from 'react-i18next';

// Recursos de traducción
const resources = {
  es: {
    translation: {
      // Login / Registro
      login: {
        title: 'Adastra Sky',
        subtitle: 'Explora los Cielos de Canarias',
        email: 'Correo electrónico',
        password: 'Contraseña',
        loginButton: 'Iniciar Sesión',
        registerButton: 'Registrarse',
        noAccount: '¿No tienes cuenta?',
        hasAccount: '¿Ya tienes cuenta?',
        atmosphere: 'Densidad Atmosférica',
      },
      // Dashboard
      dashboard: {
        title: 'Santuarios Estelares',
        search: 'Buscar santuario...',
        allIslands: 'Todas las Islas',
        observatories: 'Observatorios',
        astronomicalViewpoints: 'Miradores Astronómicos',
        landscapeViewpoints: 'Miradores Paisajísticos',
      },
      // Categorías
      categories: {
        observatory: 'Observatorio Astronómico',
        astronomical_viewpoint: 'Mirador Astronómico',
        landscape_viewpoint: 'Mirador Paisajístico',
      },
      // Ficha Técnica
      sanctuary: {
        realImage: 'Imagen Real',
        live: 'EN DIRECTO',
        altitude: 'Altitud',
        accessibility: 'Accesibilidad',
        weather: 'Meteorología',
        temperature: 'Temperatura',
        cloudiness: 'Nubosidad',
        wind: 'Viento',
        visibility: 'Visibilidad',
        humidity: 'Humedad',
        skyQuality: 'Calidad del Cielo',
        bortleScale: 'Escala Bortle',
        seeing: 'Seeing',
        lightPollution: 'Contaminación Lumínica',
        sources: 'Fuentes Consultadas',
        loading: 'Cargando',
      },
      // Chat
      chat: {
        title: 'Adastra',
        placeholder: 'Pregunta sobre los cielos de Canarias...',
        thinking: 'Pensando...',
        send: 'Enviar',
      },
      // Calendario
      calendar: {
        title: 'Calendario Astronómico',
        subtitle: 'Eventos astronómicos y reservas en observatorios',
        today: 'Hoy',
        eventsIn: 'Eventos en',
        loading: 'Cargando eventos...',
        noEvents: 'No hay eventos este mes',
        tabCalendar: 'Calendario',
        tabReservas: 'Reservas',
        apodTitle: 'Astronomy Picture of the Day (NASA)',
      },
      // Reservas
      reservas: {
        title: 'Reservar Visita a los Observatorios',
        subtitle: 'Los observatorios del IAC en Canarias ofrecen visitas guiadas. Las reservas se gestionan directamente a través de las plataformas oficiales.',
        bookVisit: 'Reservar Visita',
        officialWeb: 'Web Oficial',
        moreInfo: 'Más información',
        lessInfo: 'Menos info',
        importantTitle: 'Información Importante',
        suspended: 'Las visitas a ambos observatorios quedan suspendidas entre diciembre y marzo por condiciones meteorológicas adversas.',
        accessTeide: 'El acceso al Observatorio del Teide requiere permiso del IAC (gestionado en la reserva).',
        accessORM: 'Para el ORM (La Palma), la carretera es de alta montaña. Se recomienda consultar el estado meteorológico antes de viajar.',
        jppaa: 'Las plazas para las Jornadas de Puertas Abiertas del Teide (20-21 junio 2026) se agotan en horas. Inscripción desde el 1 de junio a las 10:00.',
      },
      // 404
      notFound: {
        message: 'Esta página no existe en el universo conocido',
        goHome: 'Volver al inicio',
      },
      // Home
      home: {
        heroTitle: 'Explora los Cielos de Canarias',
        heroSubtitle: 'Descubre los mejores lugares del mundo para observar las estrellas',
        startExploring: 'Comenzar Exploración',
        learnMore: 'Saber más',
        featuredTitle: 'Santuarios Estelares Destacados',
        chatWithAI: 'Habla con Adastra',
        chatDesc: 'Pregunta a nuestro guía astronómico inteligente sobre los cielos canarios',
      },
      // Perfil
      profile: {
        title: 'Mi Perfil',
        personalInfo: 'Información Personal',
        changePassword: 'Cambiar Contraseña',
        preferences: 'Preferencias',
        language: 'Idioma',
        saveChanges: 'Guardar Cambios',
      },
      // Admin
      admin: {
        title: 'Panel de Administración',
        zones: 'Zonas Astronómicas',
        users: 'Usuarios',
        createZone: 'Crear Zona',
        editZone: 'Editar Zona',
        deleteZone: 'Eliminar Zona',
        confirmDelete: '¿Estás seguro de eliminar esta zona?',
      },
      // Contacto
      contact: {
        title: 'Contacto',
        subtitle: 'Envíanos un mensaje y te responderemos lo antes posible',
        name: 'Nombre',
        email: 'Email',
        subject: 'Asunto',
        message: 'Mensaje',
        send: 'Enviar Mensaje',
        success: 'Mensaje enviado correctamente',
      },
      // FAQ
      faq: {
        title: 'Preguntas Frecuentes',
        search: 'Buscar pregunta...',
      },
      // Eventos
      events: {
        title: 'Eventos Astronómicos',
        filters: 'Filtros',
        allTypes: 'Todos los tipos',
        meteorShower: 'Lluvia de Estrellas',
        eclipse: 'Eclipse',
        comet: 'Cometa',
        planetary: 'Planetario',
      },
      // Observatorios
      observatories: {
        title: 'Observatorios',
        subtitle: 'Los observatorios astronómicos más importantes de Canarias',
        visit: 'Visitar',
      },
      // Datos
      data: {
        title: 'Datos Astronómicos',
        exportGeoJSON: 'Exportar GeoJSON',
        exportCSV: 'Exportar CSV',
        zones: 'Zonas',
        total: 'Total',
      },
      // Experiences
      experiences: {
        title: 'Experiencias',
        subtitle: 'Recuerdos y vivencias compartidos por la comunidad',
        share: 'Compartir experiencia',
        createTitle: 'Comparte tu experiencia',
        titleField: 'Título',
        titlePlaceholder: 'Un atardecer inolvidable...',
        description: 'Descripción',
        descriptionPlaceholder: 'Cuenta tu experiencia en este lugar...',
        location: 'Ubicación',
        selectLocation: 'Selecciona un lugar...',
        photos: 'Fotografías (máx. 5)',
        publish: 'Publicar experiencia',
        noExperiences: 'Aún no hay experiencias',
        beFirst: 'Sé el primero en compartir tu vivencia en los santuarios estelares de Canarias',
        shareMine: 'Compartir mi experiencia',
        viewAll: 'Ver todas las experiencias →',
        statsExperiences: 'Experiencias',
        statsPhotos: 'Fotografías',
        statsLocations: 'Ubicaciones',
        myGallery: 'Mi Galería',
        noGallery: 'Aún no has compartido experiencias',
        goToExperiences: 'Ir a Experiencias',
      },
      // Explorador
      explorer: {
        title: 'Explorador de Cielo',
        filters: 'Filtros',
        island: 'Isla',
        category: 'Categoría',
        allIslands: 'Todas las Islas',
      },
      // Errores
      errors: {
        generic: 'Ha ocurrido un error',
        retry: 'Intentar de nuevo',
        notFound: 'No encontrado',
        unauthorized: 'No autorizado',
      },
      // General
      common: {
        back: 'Volver',
        close: 'Cerrar',
        save: 'Guardar',
        cancel: 'Cancelar',
        loading: 'Cargando...',
        error: 'Error',
        success: 'Éxito',
        search: 'Buscar',
        filter: 'Filtrar',
        all: 'Todos',
        noData: 'No hay datos disponibles',
      },
    },
  },
  en: {
    translation: {
      login: {
        title: 'Adastra Sky',
        subtitle: 'Explore the Skies of the Canary Islands',
        email: 'Email',
        password: 'Password',
        loginButton: 'Login',
        registerButton: 'Register',
        noAccount: "Don't have an account?",
        hasAccount: 'Already have an account?',
        atmosphere: 'Atmospheric Density',
      },
      dashboard: {
        title: 'Stellar Sanctuaries',
        search: 'Search sanctuary...',
        allIslands: 'All Islands',
        observatories: 'Observatories',
        astronomicalViewpoints: 'Astronomical Viewpoints',
        landscapeViewpoints: 'Landscape Viewpoints',
      },
      categories: {
        observatory: 'Astronomical Observatory',
        astronomical_viewpoint: 'Astronomical Viewpoint',
        landscape_viewpoint: 'Landscape Viewpoint',
      },
      sanctuary: {
        realImage: 'Real Image',
        live: 'LIVE',
        altitude: 'Altitude',
        accessibility: 'Accessibility',
        weather: 'Weather',
        temperature: 'Temperature',
        cloudiness: 'Cloudiness',
        wind: 'Wind',
        visibility: 'Visibility',
        humidity: 'Humidity',
        skyQuality: 'Sky Quality',
        bortleScale: 'Bortle Scale',
        seeing: 'Seeing',
        lightPollution: 'Light Pollution',
        sources: 'Sources Consulted',
        loading: 'Loading',
      },
      chat: {
        title: 'Adastra',
        placeholder: 'Ask about the skies of the Canary Islands...',
        thinking: 'Thinking...',
        send: 'Send',
      },
      calendar: {
        title: 'Astronomical Calendar',
        subtitle: 'Astronomical events and observatory bookings',
        today: 'Today',
        eventsIn: 'Events in',
        loading: 'Loading events...',
        noEvents: 'No events this month',
        tabCalendar: 'Calendar',
        tabReservas: 'Bookings',
        apodTitle: 'Astronomy Picture of the Day (NASA)',
      },
      reservas: {
        title: 'Book a Visit to the Observatories',
        subtitle: 'The IAC observatories in the Canary Islands offer guided tours. Bookings are managed directly through the official platforms.',
        bookVisit: 'Book a Visit',
        officialWeb: 'Official Website',
        moreInfo: 'More info',
        lessInfo: 'Less info',
        importantTitle: 'Important Information',
        suspended: 'Visits to both observatories are suspended between December and March due to adverse weather conditions.',
        accessTeide: 'Access to Teide Observatory requires IAC permission (managed during booking).',
        accessORM: 'For ORM (La Palma), the road is high mountain. Check weather conditions before traveling.',
        jppaa: 'Spots for the Teide Open Days (June 20-21, 2026) sell out in hours. Registration opens June 1 at 10:00.',
      },
      notFound: {
        message: 'This page does not exist in the known universe',
        goHome: 'Back to Home',
      },
      home: {
        heroTitle: 'Explore the Skies of the Canary Islands',
        heroSubtitle: 'Discover the best places in the world for stargazing',
        startExploring: 'Start Exploring',
        learnMore: 'Learn more',
        featuredTitle: 'Featured Stellar Sanctuaries',
        chatWithAI: 'Chat with Adastra',
        chatDesc: 'Ask our intelligent astronomical guide about the Canary skies',
      },
      profile: {
        title: 'My Profile',
        personalInfo: 'Personal Information',
        changePassword: 'Change Password',
        preferences: 'Preferences',
        language: 'Language',
        saveChanges: 'Save Changes',
      },
      admin: {
        title: 'Administration Panel',
        zones: 'Astronomical Zones',
        users: 'Users',
        createZone: 'Create Zone',
        editZone: 'Edit Zone',
        deleteZone: 'Delete Zone',
        confirmDelete: 'Are you sure you want to delete this zone?',
      },
      contact: {
        title: 'Contact',
        subtitle: 'Send us a message and we will reply as soon as possible',
        name: 'Name',
        email: 'Email',
        subject: 'Subject',
        message: 'Message',
        send: 'Send Message',
        success: 'Message sent successfully',
      },
      faq: {
        title: 'Frequently Asked Questions',
        search: 'Search question...',
      },
      events: {
        title: 'Astronomical Events',
        filters: 'Filters',
        allTypes: 'All types',
        meteorShower: 'Meteor Shower',
        eclipse: 'Eclipse',
        comet: 'Comet',
        planetary: 'Planetary',
      },
      observatories: {
        title: 'Observatories',
        subtitle: 'The most important astronomical observatories of the Canary Islands',
        visit: 'Visit',
      },
      data: {
        title: 'Astronomical Data',
        exportGeoJSON: 'Export GeoJSON',
        exportCSV: 'Export CSV',
        zones: 'Zones',
        total: 'Total',
      },
      experiences: {
        title: 'Experiences',
        subtitle: 'Memories and experiences shared by the community',
        share: 'Share experience',
        createTitle: 'Share your experience',
        titleField: 'Title',
        titlePlaceholder: 'An unforgettable sunset...',
        description: 'Description',
        descriptionPlaceholder: 'Tell your experience at this place...',
        location: 'Location',
        selectLocation: 'Select a place...',
        photos: 'Photos (max. 5)',
        publish: 'Publish experience',
        noExperiences: 'No experiences yet',
        beFirst: 'Be the first to share your experience in the stellar sanctuaries of the Canary Islands',
        shareMine: 'Share my experience',
        viewAll: 'View all experiences →',
        statsExperiences: 'Experiences',
        statsPhotos: 'Photos',
        statsLocations: 'Locations',
        myGallery: 'My Gallery',
        noGallery: "You haven't shared any experiences yet",
        goToExperiences: 'Go to Experiences',
      },
      explorer: {
        title: 'Sky Explorer',
        filters: 'Filters',
        island: 'Island',
        category: 'Category',
        allIslands: 'All Islands',
      },
      errors: {
        generic: 'An error occurred',
        retry: 'Try again',
        notFound: 'Not found',
        unauthorized: 'Unauthorized',
      },
      common: {
        back: 'Back',
        close: 'Close',
        save: 'Save',
        cancel: 'Cancel',
        loading: 'Loading...',
        error: 'Error',
        success: 'Success',
        search: 'Search',
        filter: 'Filter',
        all: 'All',
        noData: 'No data available',
      },
    },
  },
  de: {
    translation: {
      login: {
        title: 'Adastra Sky',
        subtitle: 'Erkunden Sie den Himmel der Kanarischen Inseln',
        email: 'E-Mail',
        password: 'Passwort',
        loginButton: 'Anmelden',
        registerButton: 'Registrieren',
        noAccount: 'Haben Sie kein Konto?',
        hasAccount: 'Haben Sie bereits ein Konto?',
        atmosphere: 'Atmosphärendichte',
      },
      dashboard: {
        title: 'Sternheiligtümer',
        search: 'Heiligtum suchen...',
        allIslands: 'Alle Inseln',
        observatories: 'Observatorien',
        astronomicalViewpoints: 'Astronomische Aussichtspunkte',
        landscapeViewpoints: 'Landschaftsaussichtspunkte',
      },
      categories: {
        observatory: 'Astronomisches Observatorium',
        astronomical_viewpoint: 'Astronomischer Aussichtspunkt',
        landscape_viewpoint: 'Landschaftsaussichtspunkt',
      },
      sanctuary: {
        realImage: 'Echtes Bild',
        live: 'LIVE',
        altitude: 'Höhe',
        accessibility: 'Zugänglichkeit',
        weather: 'Wetter',
        temperature: 'Temperatur',
        cloudiness: 'Bewölkung',
        wind: 'Wind',
        visibility: 'Sichtweite',
        humidity: 'Feuchtigkeit',
        skyQuality: 'Himmelqualität',
        bortleScale: 'Bortle-Skala',
        seeing: 'Seeing',
        lightPollution: 'Lichtverschmutzung',
        sources: 'Konsultierte Quellen',
        loading: 'Laden...',
      },
      chat: {
        title: 'Adastra',
        placeholder: 'Fragen Sie nach dem Himmel der Kanarischen Inseln...',
        thinking: 'Denken...',
        send: 'Senden',
      },
      calendar: {
        title: 'Astronomischer Kalender',
        subtitle: 'Astronomische Ereignisse und Observatoriumsbuchungen',
        today: 'Heute',
        eventsIn: 'Ereignisse in',
        loading: 'Ereignisse werden geladen...',
        noEvents: 'Keine Ereignisse diesen Monat',
        tabCalendar: 'Kalender',
        tabReservas: 'Buchungen',
        apodTitle: 'Astronomiebild des Tages (NASA)',
      },
      reservas: {
        title: 'Besuch der Observatorien buchen',
        subtitle: 'Die IAC-Observatorien auf den Kanarischen Inseln bieten Führungen an. Buchungen werden direkt über die offiziellen Plattformen verwaltet.',
        bookVisit: 'Besuch buchen',
        officialWeb: 'Offizielle Website',
        moreInfo: 'Mehr Info',
        lessInfo: 'Weniger Info',
        importantTitle: 'Wichtige Informationen',
        suspended: 'Besuche beider Observatorien sind von Dezember bis März aufgrund widriger Wetterbedingungen ausgesetzt.',
        accessTeide: 'Der Zugang zum Teide-Observatorium erfordert eine IAC-Genehmigung (bei der Buchung geregelt).',
        accessORM: 'Für das ORM (La Palma) ist die Straße eine Hochgebirgsstraße. Überprüfen Sie die Wetterbedingungen vor der Reise.',
        jppaa: 'Die Plätze für die Teide-Tage der offenen Tür (20.-21. Juni 2026) sind in Stunden ausverkauft. Anmeldung ab 1. Juni um 10:00 Uhr.',
      },
      notFound: {
        message: 'Diese Seite existiert im bekannten Universum nicht',
        goHome: 'Zurück zur Startseite',
      },
      home: {
        heroTitle: 'Entdecken Sie den Himmel der Kanarischen Inseln',
        heroSubtitle: 'Entdecken Sie die besten Orte der Welt zur Sternenbeobachtung',
        startExploring: 'Erkundung starten',
        learnMore: 'Mehr erfahren',
        featuredTitle: 'Ausgewählte Sternheiligtümer',
        chatWithAI: 'Mit Adastra chatten',
        chatDesc: 'Fragen Sie unseren intelligenten astronomischen Führer über den kanarischen Himmel',
      },
      profile: {
        title: 'Mein Profil',
        personalInfo: 'Persönliche Informationen',
        changePassword: 'Passwort ändern',
        preferences: 'Einstellungen',
        language: 'Sprache',
        saveChanges: 'Änderungen speichern',
      },
      admin: {
        title: 'Administrationspanel',
        zones: 'Astronomische Zonen',
        users: 'Benutzer',
        createZone: 'Zone erstellen',
        editZone: 'Zone bearbeiten',
        deleteZone: 'Zone löschen',
        confirmDelete: 'Sind Sie sicher, dass Sie diese Zone löschen möchten?',
      },
      contact: {
        title: 'Kontakt',
        subtitle: 'Senden Sie uns eine Nachricht und wir antworten so schnell wie möglich',
        name: 'Name',
        email: 'E-Mail',
        subject: 'Betreff',
        message: 'Nachricht',
        send: 'Nachricht senden',
        success: 'Nachricht erfolgreich gesendet',
      },
      faq: {
        title: 'Häufig gestellte Fragen',
        search: 'Frage suchen...',
      },
      events: {
        title: 'Astronomische Ereignisse',
        filters: 'Filter',
        allTypes: 'Alle Typen',
        meteorShower: 'Meteorschauer',
        eclipse: 'Finsternis',
        comet: 'Komet',
        planetary: 'Planetarisch',
      },
      observatories: {
        title: 'Observatorien',
        subtitle: 'Die wichtigsten astronomischen Observatorien der Kanarischen Inseln',
        visit: 'Besuchen',
      },
      data: {
        title: 'Astronomische Daten',
        exportGeoJSON: 'GeoJSON exportieren',
        exportCSV: 'CSV exportieren',
        zones: 'Zonen',
        total: 'Gesamt',
      },
      experiences: {
        title: 'Erfahrungen',
        subtitle: 'Erinnerungen und Erlebnisse der Community',
        share: 'Erfahrung teilen',
        createTitle: 'Teile deine Erfahrung',
        titleField: 'Titel',
        titlePlaceholder: 'Ein unvergesslicher Sonnenuntergang...',
        description: 'Beschreibung',
        descriptionPlaceholder: 'Erzähle von deinem Erlebnis an diesem Ort...',
        location: 'Standort',
        selectLocation: 'Wähle einen Ort...',
        photos: 'Fotos (max. 5)',
        publish: 'Erfahrung veröffentlichen',
        noExperiences: 'Noch keine Erfahrungen',
        beFirst: 'Sei der Erste, der deine Erfahrung in den Sternenheiligtümern der Kanarischen Inseln teilt',
        shareMine: 'Meine Erfahrung teilen',
        viewAll: 'Alle Erfahrungen anzeigen →',
        statsExperiences: 'Erfahrungen',
        statsPhotos: 'Fotos',
        statsLocations: 'Standorte',
        myGallery: 'Meine Galerie',
        noGallery: 'Du hast noch keine Erfahrungen geteilt',
        goToExperiences: 'Gehe zu Erfahrungen',
      },
      explorer: {
        title: 'Himmelsforscher',
        filters: 'Filter',
        island: 'Insel',
        category: 'Kategorie',
        allIslands: 'Alle Inseln',
      },
      errors: {
        generic: 'Ein Fehler ist aufgetreten',
        retry: 'Erneut versuchen',
        notFound: 'Nicht gefunden',
        unauthorized: 'Nicht autorisiert',
      },
      common: {
        back: 'Zurück',
        close: 'Schließen',
        save: 'Speichern',
        cancel: 'Abbrechen',
        loading: 'Laden...',
        error: 'Fehler',
        success: 'Erfolg',
        search: 'Suchen',
        filter: 'Filtern',
        all: 'Alle',
        noData: 'Keine Daten verfügbar',
      },
    },
  },
};

i18n
  .use(initReactI18next)
  .init({
    resources,
    lng: 'es',
    fallbackLng: 'es',
    interpolation: {
      escapeValue: false,
    },
  });

export default i18n;
